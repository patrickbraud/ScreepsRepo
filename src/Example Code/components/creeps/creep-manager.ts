...
export namespace CreepManager {
    ...
    export function loadCreeps(): void {
        ...
        if (Config.VERBOSE) {
            console.log(this.creepCount + " creeps alive.");
        }
    }
    ...
    export function createHarvester(): number {
        ...
        var canCreateStatus: number = SpawnManager.getFirstSpawn().canCreateCreep(bodyParts, name);
        if (canCreateStatus == OK) {
            SpawnManager.getFirstSpawn().createCreep(bodyParts, name, properties);
